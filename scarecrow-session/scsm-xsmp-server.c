...
#define SCSM_ICE_AUTH_RETRIES      10
#define SCSM_ICE_AUTH_INTERVAL     2   /* 2 seconds */
#define SCSM_ICE_AUTH_LOCK_TIMEOUT 600 /* 10 minutes */
...
static void
scsm_xsmp_server_set_property (GObject      *object,
                              guint         prop_id,
                              const GValue *value,
                              GParamSpec   *pspec)
{
        ...
        case PROP_CLIENT_STORE:
                scsm_xsmp_server_set_client_store (self, g_value_get_object (value));
                break;
         default:
...
static void
scsm_xsmp_server_get_property (GObject    *object,
                              guint       prop_id,
                              GValue     *value,
                              GParamSpec *pspec)
{
                        ...
                        char         **failure_reason_ret)
{
        ...
        scsm_store_add (server->priv->client_store, scsm_client_peek_id (client), G_OBJECT (client));
        /* the store will own the ref */
        g_object_unref (client);

        scsm_xsmp_client_connect (SCSM_XSMP_CLIENT (client), sms_conn, mask_ret, callbacks_ret);
                ...
                const char *network_id)
{
        ...
        file_entry->auth_name = strdup (SCSM_ICE_MAGIC_COOKIE_AUTH_NAME);
        file_entry->auth_data = IceGenerateMagicCookie (SCSM_ICE_MAGIC_COOKIE_LEN);
        file_entry->auth_data_length = SCSM_ICE_MAGIC_COOKIE_LEN;
                     ...
                     gboolean       adding)
{
        ...
        do {
                ret = IceLockAuthFile (filename,
                                       SCSM_ICE_AUTH_RETRIES,
                                       SCSM_ICE_AUTH_INTERVAL,
                                       SCSM_ICE_AUTH_LOCK_TIMEOUT);
...
setup_listener (ScsmXsmpServer *server)
{
        ...
        res = SmsInitialize (PACKAGE,
                             ...
                             error);
        if (! res) {
                scsm_util_init_error (TRUE, "Could not initialize libSM: %s", error);
        }
        ...
        res = IceListenForConnections (&server->priv->num_xsmp_sockets,
                                       ...
                                       error);
        if (! res) {
                scsm_util_init_error (TRUE, _("Could not create ICE listening socket: %s"), error);
        }
        ...
        if (server->priv->num_local_xsmp_sockets == 0) {
                scsm_util_init_error (TRUE, "IceListenForConnections did not return a local listener!");
        }
...
static GObject *
scsm_xsmp_server_constructor (GType                  type,
...
scsm_xsmp_server_class_init (ScsmXsmpServerClass *klass)
{
        ...
        object_class->get_property = scsm_xsmp_server_get_property;
        object_class->set_property = scsm_xsmp_server_set_property;
        object_class->constructor = scsm_xsmp_server_constructor;
        object_class->finalize = scsm_xsmp_server_finalize;
...
scsm_xsmp_server_finalize (GObject *object)
{
        ...
        xsmp_server = SCSM_XSMP_SERVER (object);
...
scsm_xsmp_server_new (ScsmStore *client_store)
{
        if (xsmp_server_object != NULL) {
                ...
                xsmp_server_object = g_object_new (SCSM_TYPE_XSMP_SERVER,
        ...
        return SCSM_XSMP_SERVER (xsmp_server_object);
}
