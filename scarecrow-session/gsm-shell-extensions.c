...
#define SHELL_SCHEMA "io.github.scarecrow-de.shell"
...
gsm_shell_extensions_scan (GsmShellExtensions *self)
{
  ...
  /* User data dir first. */
  dirname = g_build_filename (g_get_user_data_dir (), "scarecrow-shell", "extensions", NULL);
  ...
  for (i = 0; system_data_dirs[i]; i++)
    {
      dirname = g_build_filename (system_data_dirs[i], "scarecrow-shell", "extensions", NULL);
      dir = g_file_new_for_path (dirname);
      g_free (dirname);
