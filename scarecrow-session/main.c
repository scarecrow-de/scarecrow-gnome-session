...
{
        int status;
        char *argv[] = { LIBEXECDIR "/scarecrow-session-check-accelerated", NULL };
 ...
 * instance (currently scarecrow-session-ctl@monitor.service):
 *
...
main (int argc, char **argv)
{
        ...
        error = NULL;
        options = g_option_context_new (_(" — the Scarecrow session manager"));
         ...
         * journald picks ups the nicer "scarecrow-session" as the program
        ...
        if (use_systemd && !systemd_service) {
                g_autoptr(GError) error = NULL;
                g_autofree gchar *scarecrow_session_target;
                ...
                scarecrow_session_target = g_strdup_printf ("scarecrow-session-%s@%s.target", session_type, session_name);
                if (gsm_util_start_systemd_unit (scarecrow_session_target, "fail", &error)) {
        ...
        /* We want to use the Scarecrow menus which has the designed categories.
         */
