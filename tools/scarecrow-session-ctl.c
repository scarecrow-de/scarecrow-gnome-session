 ...
 * scarecrow-session-tl.c - Small utility program to manage scarecrow systemd session.
...
#define GSM_SERVICE_DBUS   "io.github.scarecrow-de.SessionManager"
#define GSM_PATH_DBUS      "/io/github/scarecrow-de/SessionManager"
#define GSM_INTERFACE_DBUS "io.github.scarecrow-de.SessionManager"
...
do_monitor_leader (void)
{
        ...
        fifo_name = g_strdup_printf ("%s/scarecrow-session-leader-fifo",
...
main (int argc, char *argv[])
{
        ...
        static const GOptionEntry options[] = {
                { "shutdown", '\0', 0, G_OPTION_ARG_NONE, &opt_shutdown, N_("Start scarecrow-session-shutdown.target"), NULL },
                { "monitor", '\0', 0, G_OPTION_ARG_NONE, &opt_monitor, N_("Start scarecrow-session-shutdown.target when receiving EOF or a single byte on stdin"), NULL },
                { "signal-init", '\0', 0, G_OPTION_ARG_NONE, &opt_signal_init, N_("Signal initialization done to scarecrow-session"), NULL },
                { "restart-dbus", '\0', 0, G_OPTION_ARG_NONE, &opt_restart_dbus, N_("Restart dbus.service if it is running"), NULL },
                { "exec-stop-check", '\0', 0, G_OPTION_ARG_NONE, &opt_exec_stop_check, N_("Run from ExecStopPost to start scarecrow-session-failed.target on service failure"), NULL },
                { NULL },
        };
        ...
        if (opt_signal_init) {
                ...
                do_start_unit ("scarecrow-session-shutdown.target", "replace-irreversibly");
        } else if (opt_monitor) {
                do_monitor_leader ();
                do_start_unit ("scarecrow-session-shutdown.target", "replace-irreversibly");
                ...
                if (g_strcmp0 ("start-limit-hit", g_getenv ("SERVICE_RESULT")) == 0) {
                        do_start_unit ("scarecrow-session-failed.target", "fail");
                }
       } else {
