...
#include "scarecrow-session-check-accelerated-common.h"
...
main (int argc, char **argv)
{
        ...
        int estatus;
        char *gl_helper_argv[] = { LIBEXECDIR "/scarecrow-session-check-accelerated-gl-helper", "--print-renderer", NULL };
        char *gles_helper_argv[] = { LIBEXECDIR "/scarecrow-session-check-accelerated-gles-helper", "--print-renderer", NULL };
        ...
        /* scarecrow-session-check-accelerated gets run before X is started in the wayland
        ...
        if (!GDK_IS_X11_DISPLAY (display)) {
                g_printerr ("scarecrow-session-check-accelerated: no X11 display found\n");
                return 1;
        }
 ...
 read:
        ...
        if (g_spawn_sync (NULL, (char **) gl_helper_argv, NULL, 0,
                ...
                g_printerr ("scarecrow-session-check-accelerated: GL Helper exited with code %d\n", estatus);
        }
        ...
        if (g_spawn_sync (NULL, (char **) gles_helper_argv, NULL, 0,
                ...
                g_printerr ("scarecrow-session-check-accelerated: GLES Helper exited with code %d\n", estatus);
        }
        ...
        if (gl_error != NULL) {
                g_printerr ("scarecrow-session-check-accelerated: Failed to run GL helper: %s\n", gl_error->message);
                g_clear_error (&gl_error);
        }

        if (gles_error != NULL) {
                g_printerr ("scarecrow-session-check-accelerated: Failed to run GLES helper: %s\n", gles_error->message);
