...
{
        return g_build_filename (g_get_user_config_dir (), "scarecrow-session", name, NULL);
}
...
populate_session_list (GtkWidget *session_list)
{
        ...
        path = g_build_filename (g_get_user_config_dir (), "scarecrow-session", NULL);
        error = NULL;
...
make_session_current (const char *name)
{
        ...
        path1 = g_build_filename (g_get_user_config_dir (), "scarecrow-session", "saved-session", NULL);
...
create_and_select_session (const char *name)
{
        ...
        path = g_build_filename (g_get_user_config_dir (), "scarecrow-session", name, NULL);
...
auto_save_next_session_if_needed (void)
{
        ...
        marker = g_build_filename (g_get_user_config_dir (),
                                   "scarecrow-session", "saved-session",
                                   ".new-session", NULL);
