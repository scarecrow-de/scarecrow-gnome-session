/* gcc -o scarecrow-session-accelerated `pkg-config --cflags --libs xcomposite gl` -Wall scarecrow-session-is-accelerated.c */
 ...
 *    scarecrow-session to directly try and do any GL detection; better to
 *    use a helper binary.
 *
...
{
        fprintf (stderr, "scarecrow-session-is-accelerated: %s\n", str);
}
